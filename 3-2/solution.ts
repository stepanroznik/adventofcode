...
const gearRatios = array.map((line, lineIndex) => {
    ...
    for (const asterisk of asteriskMatches) {
        ...
        // Define the relative positions in the specified order
        const positions: [number, number][] = [
            [1, -1], [0, -1], [-1, -1],  // Bottom Left, Mid Left, Top Left
            [-1, 0], [-1, 1],            // Top Middle, Top Right
            [0, 1], [1, 1], [1, 0]       // Mid Right, Bottom Right, Bottom Middle
        ];

        const neighbors = positions.map(([y, x]) => array[lineIndex + y]?.[gearIndex + x]);

        const validNumbers: { number: number, positions: [number, number][] }[] = [];
        ...
        neighbors.forEach((symbol, positionIndex) => {
            if (!isNaN(+symbol)) {
                ...
                validNumbers.push({ number: +numberDigits.join(''), positions: digitPositions })
            }
        })

        // Remove duplicates
        const stringPositions = validNumbers.map(({ positions }) => JSON.stringify(positions));
        const uniqueValidNumbers = validNumbers.filter(({ positions }, index) => 
            !stringPositions.includes(JSON.stringify(positions), index + 1));
        
        if (uniqueValidNumbers.length === 2) lineRatios.push(uniqueValidNumbers[0].number * uniqueValidNumbers[1].number)
    }
    return lineRatios;
})
